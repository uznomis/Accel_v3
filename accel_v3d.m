function varargout = accel_v3d(varargin)
% ACCEL_V3D MATLAB code for accel_v3d.fig
%      ACCEL_V3D, by itself, creates a new ACCEL_V3D or raises the existing
%      singleton*.
%
%      H = ACCEL_V3D returns the handle to a new ACCEL_V3D or the handle to
%      the existing singleton*.
%
%      ACCEL_V3D('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ACCEL_V3D.M with the given input arguments.
%
%      ACCEL_V3D('Property','Value',...) creates a new ACCEL_V3D or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before accel_v3d_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to accel_v3d_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help accel_v3d

% Last Modified by GUIDE v2.5 18-Nov-2016 15:03:14

% Begin initialization code - DO NOT EDIT
gui_Singleton = 0;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @accel_v3d_OpeningFcn, ...
    'gui_OutputFcn',  @accel_v3d_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before accel_v3d is made visible.
function accel_v3d_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to accel_v3d (see VARARGIN)

% Choose default command line output for accel_v3d
handles.output = hObject;
% Update handles structure
guidata(hObject, handles);

% UIWAIT makes accel_v3d wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = accel_v3d_OutputFcn(hObject, eventdata, handles)
...
for i = 1:length(SN)
    plot(handles.axes1,ones(1,2)*start(i),yl,...
        'Color','black');
    text(handles.axes1,start(i),getPlotHeight(i,yl),num2str(SN(i)),...
        'FontSize',20);
end
hold(handles.axes1,'off');
